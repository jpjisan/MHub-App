...
function Movie() {
  ...
  const getMovie = async (pageNum = page) => {
    // setloading(true);
    try {
      let { data } = await axios.get(`/movie/${category}?page=${pageNum}`);
  ...
  const refreshTrending = async () => {
    ...
    setmovie([]);
    sethasMore(true);
    await getMovie(1); // Always fetch page 1 after reset
  };
  useEffect(() => {
    refreshTrending();
  ...
  return !movie ? (
    ...
    <div className="w-[100vw]  min-h-screen p-1 bg-[#1f1e24]">
